...
func getNameCounts(names []string) map[rune]map[string]int {
	// ?
	alphabetToNamesToCounts := make(map[rune]map[string]int)

	for _, name := range names {
		r := []rune(name)[0]
		if _, ok := alphabetToNamesToCounts[r]; !ok {
			alphabetToNamesToCounts[r] = make(map[string]int)
		}

		namesToCounts := alphabetToNamesToCounts[r]
		if _, ok := namesToCounts[name]; !ok {
			namesToCounts[name] = 0
		}

		namesToCounts[name]++
	}

	return alphabetToNamesToCounts
}
